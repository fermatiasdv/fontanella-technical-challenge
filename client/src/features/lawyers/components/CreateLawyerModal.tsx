...
import type { CreateLawyerDto, LawyerAPI, ScheduleSlotInput, VacationInput } from '@/features/lawyers/types/lawyer.types';
...
export interface CreateLawyerModalProps {
  isOpen:        boolean;
  onClose:       () => void;
  onSubmit: (
    dto:       CreateLawyerDto,
    contacts:  ReturnType<typeof getActiveContacts>,
...
function validate(form: FormState): FormErrors {
  ...
  else if (form.full_name.trim().length < 3) errors.full_name = 'Debe tener al menos 3 caracteres.';
  if (!form.national_id.trim()) errors.national_id = 'El DNI es requerido.';
  else if (!/^[\d.\-/]+$/.test(form.national_id.trim())) errors.national_id = 'Solo dígitos, puntos, guiones y barras.';
  if (!form.location.trim()) errors.location = 'La ubicación es requerida.';
  if (!form.timezone) errors.timezone = 'Seleccioná un huso horario.';
  return errors;
}
...
function scheduleIsValid(schedule: ScheduleState): boolean {
  return Object.values(schedule).some((d) => d.active && Boolean(d.startTime) && Boolean(d.endTime));
}
...
function ScheduleSection({ schedule, onChange, scheduleError }: { schedule: ScheduleState; onChange: (s: ScheduleState) => void; scheduleError?: string }) {
  ...
  return (
    <div className={`section-card${scheduleError ? ' section-card--error' : ''}`}>
      <div className="section-card__header">
        ...
        Horario de trabajo
        <span style={{ marginLeft: 'auto', fontWeight: 400, textTransform: 'none', letterSpacing: 'normal', color: 'var(--c-error)', opacity: 0.8 }}>Requerido</span>
      </div>
      <div className="section-card__body">
        <div className="day-selector">
          {DAYS.map(({ key, short }) => {
            ...
            return (
              <button key={key} type="button" title={DAYS.find((d) => d.key === key)?.label} className={pillClass} onClick={() => setSelectedKey(key)}>
        ...
        <div className="day-editor">
          <div className="day-editor__header">
            ...
            <label className="day-editor__toggle">
              <input type="checkbox" checked={current.active} onChange={(e) => setDayField('active', e.target.checked)} />
          ...
          {current.active ? (
            <div className="form-grid-2">
              <div className="form-field">
                <label className="form-field__label"><span className="material-symbols-outlined">login</span>Entrada</label>
                <input type="time" className="form-input" min="06:00" max="22:00" value={current.startTime} onChange={(e) => setDayField('startTime', e.target.value)} />
              </div>
              <div className="form-field">
                <label className="form-field__label"><span className="material-symbols-outlined">logout</span>Salida</label>
                <input type="time" className="form-input" min="06:00" max="22:00" value={current.endTime} onChange={(e) => setDayField('endTime', e.target.value)} />
              </div>
            </div>
          ) : (
            <p className="day-editor__rest"><span className="material-symbols-outlined">block</span>Sin citas disponibles este día.</p>
          )}
        </div>
        {scheduleError && <p className="form-field__error"><span className="material-symbols-outlined">error</span>{scheduleError}</p>}
      </div>
    </div>
  );
}

function VacationsSection({ rows, onAdd, onRemove, onChange }: { rows: VacationRow[]; onAdd: () => void; onRemove: (id: string) => void; onChange: (id: string, field: 'startDate' | 'endDate', value: string) => void }) {
  return (
    <div className="section-card">
      <div className="section-card__header">
        <span className="material-symbols-outlined" style={{ fontSize: '0.875rem' }}>beach_access</span>
        Vacaciones
        <span style={{ marginLeft: 'auto', fontWeight: 400, textTransform: 'none', letterSpacing: 'normal', opacity: 0.6 }}>Opcional</span>
        <button type="button" className="btn-add-row" onClick={onAdd} style={{ marginLeft: '0.5rem' }}>
          <span className="material-symbols-outlined" style={{ fontSize: '0.875rem' }}>add</span>Agregar
        </button>
      </div>
      <div className="section-card__body">
        {rows.length === 0 ? (
          ...
          <div className="vacation-rows">
            {rows.map((row) => (
              <div key={row.id} className="vacation-row">
                <div className="form-field">
                  <label className="form-field__label"><span className="material-symbols-outlined">event</span>Desde</label>
                  <input type="date" className="form-input" value={row.startDate} onChange={(e) => onChange(row.id, 'startDate', e.target.value)} />
                </div>
                <div className="form-field">
                  <label className="form-field__label"><span className="material-symbols-outlined">event</span>Hasta</label>
                  <input type="date" className="form-input" value={row.endDate} min={row.startDate || undefined} onChange={(e) => onChange(row.id, 'endDate', e.target.value)} />
                </div>
                <button type="button" className="btn-remove-row" onClick={() => onRemove(row.id)} title="Eliminar período">
...
export function CreateLawyerModal({ isOpen, onClose, onSubmit, initialLawyer }: CreateLawyerModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    setStep(1); setErrors({}); setScheduleError(undefined); setSubmitError(null);

    if (initialLawyer) {
      setForm({ full_name: initialLawyer.full_name, national_id: initialLawyer.national_id, location: initialLawyer.location, timezone: initialLawyer.timezone });
      setContacts(EMPTY_CONTACTS);
      setLoadingContacts(true);
      contactService.listByLawyer(initialLawyer.id_lawyer)
        .then((existing) => {
      ...
      workingScheduleService.getByLawyer(initialLawyer.id_lawyer)
        .then((slots) => {
          const s: ScheduleState = DAYS.reduce<ScheduleState>((acc, { key }) => { acc[key] = { active: false, startTime: '09:00', endTime: '18:00' }; return acc; }, {});
          slots.forEach((slot) => { s[slot.day_of_week] = { active: true, startTime: slot.start_time.slice(0, 5), endTime: slot.end_time.slice(0, 5) }; });
          setSchedule(s);
        })
        .catch(() => {}).finally(() => setLoadingSchedule(false));

      setVacationRows([]);
      vacationsService.getByLawyer(initialLawyer.id_lawyer)
        .then((vacs) => { setVacationRows(vacs.map((v) => ({ id: `vac-${v.id_vacation}`, startDate: v.start_date, endDate: v.end_date }))); })
        .catch(() => {});
    } else {
      setForm(EMPTY_FORM); setContacts(EMPTY_CONTACTS); setSchedule(EMPTY_SCHEDULE); setVacationRows([]);
    }
  ...
  const handleAddVacation = useCallback(() => { setVacationRows((prev) => [...prev, { id: `vac-${Date.now()}`, startDate: '', endDate: '' }]); }, []);
  const handleRemoveVacation = useCallback((id: string) => { setVacationRows((prev) => prev.filter((r) => r.id !== id)); }, []);
  const handleVacationChange = useCallback((id: string, field: 'startDate' | 'endDate', value: string) => {
    setVacationRows((prev) => prev.map((r) => (r.id === id ? { ...r, [field]: value } : r)));
  }, []);
  ...
  const initials  = form.full_name.trim().split(/\s+/).slice(0, 2).map((n) => n[0]).join('').toUpperCase();
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!hasValidSchedule) { setScheduleError('Activá al menos un día y completá los horarios.'); return; }
    const scheduleSlots: ScheduleSlotInput[] = Object.entries(schedule)
      .filter(([, d]) => d.active && d.startTime && d.endTime)
      .map(([day, d]) => ({ dayOfWeek: day, startTime: `${d.startTime}:00`, endTime: `${d.endTime}:00` }));
    const vacationInputs: VacationInput[] = vacationRows
      .filter((r) => r.startDate && r.endDate)
      .map((r) => ({ startDate: r.startDate, endDate: r.endDate }));
    setSubmitting(true); setSubmitError(null);
    try {
      await onSubmit({ full_name: form.full_name.trim(), national_id: form.national_id.trim(), location: form.location.trim(), timezone: form.timezone }, activeContacts, scheduleSlots, vacationInputs);
      onClose();
    } catch (err) {
  ...
  return (
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-card">
        <div className="modal-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <div style={{ width: '3rem', height: '3rem', background: 'linear-gradient(135deg, var(--c-primary), var(--c-primary-container))', borderRadius: 'var(--r-xl)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span className="material-symbols-outlined" style={{ color: 'white', fontSize: '1.5rem', fontVariationSettings: "'FILL' 1" }}>{isEditMode ? 'edit' : 'person_add'}</span>
            </div>
            <div>
              <p className="eyebrow" style={{ color: 'var(--c-primary)', marginBottom: '0.125rem' }}>Management Portal</p>
              <h2 style={{ fontFamily: 'var(--font-headline)', fontWeight: 800, color: 'var(--c-on-surface)', fontSize: '1.5rem', letterSpacing: '-0.02em' }}>{isEditMode ? 'Edit Practitioner' : 'New Practitioner'}</h2>
            </div>
          </div>
        ...
        <form onSubmit={step === 1 ? handleNext : handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          <div className="modal-body">
            {step === 1 && (
              <>
                <Field label="Full Name" icon="badge" error={errors.full_name}>
                  <input type="text" className="form-input" placeholder="e.g. Horacio Altamirano" value={form.full_name} onChange={setField('full_name')} autoFocus />
                </Field>
                <div className="form-grid-2">
                  <Field label="National ID (DNI)" icon="fingerprint" error={errors.national_id}>
                    <input type="text" className="form-input" placeholder="e.g. 28.495.102" value={form.national_id} onChange={setField('national_id')} />
                  </Field>
                  <Field label="Location" icon="location_on" error={errors.location}>
                    <input type="text" className="form-input" placeholder="e.g. Buenos Aires, AR" value={form.location} onChange={setField('location')} />
                  </Field>
                </div>
                ...
                {loadingContacts ? (
                  <div className="loading-contacts"><span className="material-symbols-outlined anim-spin">progress_activity</span>Loading contact methods…</div>
                ) : (
            ...
            {step === 2 && (
              <>
                <ScheduleSection schedule={schedule} onChange={(s) => { setSchedule(s); setScheduleError(undefined); }} scheduleError={scheduleError} />
                <VacationsSection rows={vacationRows} onAdd={handleAddVacation} onRemove={handleRemoveVacation} onChange={handleVacationChange} />
              </>
            )}
          ...
          <div className="modal-footer modal-footer--between">
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              {step === 2 ? (
                <button type="button" onClick={() => setStep(1)} className="btn-secondary"><span className="material-symbols-outlined">arrow_back</span>Volver</button>
              ) : (
                <button type="button" onClick={onClose} className="btn-secondary">Cancelar</button>
              )}
              {step === 1 && <button type="submit" disabled={!canGoNext} className="btn-primary">Siguiente<span className="material-symbols-outlined">arrow_forward</span></button>}
              {step === 2 && (
                <button type="submit" disabled={!canSubmit} className="btn-primary">
                  {submitting
                    ? <><span className="material-symbols-outlined anim-spin">progress_activity</span>Guardando…</>
                    : <><span className="material-symbols-outlined">{isEditMode ? 'save' : 'person_add'}</span>{isEditMode ? 'Guardar cambios' : 'Add Practitioner'}</>}
                </button>
              )}
            </div>
          </div>
