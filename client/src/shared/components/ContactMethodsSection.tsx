...
export const IN_PERSON_CITIES = [
  'Buenos Aires', 'Santiago', 'Madrid', 'Ciudad de México',
  'Tokyo', 'Prusia', 'New York', 'London', 'São Paulo', 'Paris',
] as const;
...
export function getActiveContacts(contacts: ContactsState) {
  return METHOD_CONFIGS
    .filter((m) => contacts[m.type].enabled && contacts[m.type].value.trim())
    .map((m, index) => ({
      method_type: m.type as MethodType,
...
export function ContactMethodsSection({ contacts, onToggle, onValueChange, activeCount }: Props) {
  return (
    <div className="contact-methods">
      ...
      <div className="contact-methods__list">
        {METHOD_CONFIGS.map((method) => {
          ...
          return (
            <div key={method.type} className={`contact-method${state.enabled ? ' contact-method--enabled' : ''}`}>
              <button
                type="button"
              ...
              >
                {state.enabled && (
                  <span className="material-symbols-outlined" style={{ color: 'white', fontSize: '0.875rem', fontVariationSettings: "'FILL' 1, 'wght' 600" }}>
                    check
                  </span>
                )}
              </button>
